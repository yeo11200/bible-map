...
type RomansReaderProps = {
  chapters: Record<string, { verse: number; text: string }[]>;
};

export function RomansReader({ chapters }: RomansReaderProps) {
  ...
  return (
    <section className="romans-reader" aria-label="로마서 전체 본문">
      <h2>로마서 전체</h2>
      <div className="chapter-picker" aria-label="로마서 장 선택">
        {chapterNumbers.map((number) => (
          <button
            key={number}
            type="button"
            aria-pressed={chapter === number}
            onClick={() => handleChapterChange(number)}
          >
            {number}장
          </button>
        ))}
